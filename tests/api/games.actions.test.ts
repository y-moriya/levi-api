...
Deno.test({
  ...
  async fn() {
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, werewolfAuth.token),
    );
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, bodyguardAuth.token),
    );
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, villagerAuth.token),
    );
...
Deno.test({
  ...
  async fn() {
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, player3Auth.token),
    );
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, player4Auth.token),
    );
    ...
    await consumeResponse<GameResponse>(
      await apiRequest("POST", `/games/${gameId}/join`, undefined, player5Auth.token),
    );
    ...
    await gameModel.gameStore.update(gameInstance);
    const attackResponse = await apiRequest(
      "POST",
      `/games/${gameId}/attack`,
