...
Deno.test({
  ...
  async fn() {
    ...
    const gameResponse = await apiRequest(
      "POST",
      "/games",
      { name: "Private Message Test", maxPlayers: 5 },
      ownerAuth.token,
    );
    ...
    const privateMessageResponse = await apiRequest("POST", `/games/${gameId}/chat`, {
    ...
    const deadPrivateMessageResponse = await apiRequest("POST", `/games/${gameId}/chat`, {
