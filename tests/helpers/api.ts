export { BASE_URL, TEST_PORT, testServer } from "./server.ts";
export { ApiClient } from "./client.ts";
export { consumeResponse } from "./response.ts";
export { waitForActionInitialization, waitForCondition, waitForGamePhase } from "./waiters.ts";

// 後方互換のためのラッパーとグローバルクライアント
import { BASE_URL } from "./server.ts";
import { ApiClient, apiRequest as rawApiRequest } from "./client.ts";
import {
  createAuthenticatedUser as _createAuthenticatedUser,
  createTestUser as _createTestUser,
  loginTestUser as _loginTestUser,
} from "./users.ts";
