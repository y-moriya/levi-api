...
export function assignRoles(game: Game): GamePlayer[] {
  ...
  logger.debug(
    `役職割り当て開始: 人狼=${game.settings.roles.werewolfCount}人, 占い師=${game.settings.roles.seerCount}人, 狩人=${game.settings.roles.bodyguardCount}人, 霊能者=${game.settings.roles.mediumCount}人`,
  );
  ...
  logger.debug(
    `役職割り当て結果: 人狼=${werewolvesCount}人, 占い師=${seersCount}人, 狩人=${bodyguardsCount}人, 霊能者=${mediumsCount}人, 村人=${villagersCount2}人`,
  );
  ...
  if (
    ...
    return forceRoleAssignment(
      game.players,
      expected.werewolfCount,
      expected.seerCount,
      expected.bodyguardCount,
      expected.mediumCount,
    );
  }
